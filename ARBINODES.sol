...
contract ARBINODES is ERC721, Ownable {
    using SafeMath for uint256;
    uint256 public constant MAX_ARBINODES = 10000;
    bool public hasSaleStarted = false;
    string public metaIpfsHash = "";

    constructor() ERC721("ARBINODES", "ARBN") {
        setBaseURI("https://www.arbinodes.art/api/node/");
    }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_ARBINODES, "Sale has ended.");
        return 90000000000000000; //0.09 AETH 
    }

    function rollupNODES(uint256 numNodes) public payable {
        require(totalSupply() < MAX_ARBINODES, "Sale has ended.");
        require(
            numNodes > 0 && numNodes <= 100,
            "Minumum Purchase Amount is 1 ARBINODES, Maximum Purchase Amount is 100 ARBINODES."
        );
        require(
            totalSupply().add(numNodes) <= MAX_ARBINODES,
            "Exceeds the Supply Amount."
        );
        require(
            msg.value >= calculatePrice().mul(numNodes),
            "AETH value is not sufficient."
        );
